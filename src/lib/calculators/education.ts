...
export const educationCalculators: CalculatorDefinition[] = [
  // 1. CGPA CALCULATOR
  {
    ...
    relatedIds: ['sgpa-calculator', 'gpa-calculator', 'cgpa-to-percentage-converter', 'attendance-calculator'],
  ...
  // 2. SGPA CALCULATOR
  {
    ...
    relatedIds: ['cgpa-calculator', 'gpa-calculator', 'marks-percentage-calculator'],
    disclaimerType: 'standard'
  },
  ...
  // 3. PERCENTAGE CALCULATOR (Comprehensive)
  {
    ...
    relatedIds: ['marks-percentage-calculator', 'cgpa-to-percentage-converter', 'discount-calculator'],
    disclaimerType: 'standard'
  },
  ...
  // 4. ATTENDANCE CALCULATOR
  {
    ...
    calculate: (inputs) => {
      ...
      if (currentPercent >= target) {
        // Can bunk: (attended) / (total + x) >= targetFraction => x = floor(attended / targetFraction - total)
        ...
        // Must attend: (attended + x) / (total + x) >= targetFraction => x = ceil((targetFraction * total - attended) / (1 - targetFraction))
  ...
  // 6. REQUIRED MARKS CALCULATOR
  {
    ...
    calculate: (inputs) => {
      ...
      // target = (current * currentWeight) + (required * finalWeightFraction)
      // required = (target - (current * currentWeight)) / finalWeightFraction
