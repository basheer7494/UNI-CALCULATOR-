        ...
        <section className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 space-y-2">
            ...
            <div className="space-y-1.5 pt-1">
              {calculator.example.steps.map((stepItem: any, idx: number) => (
                <div key={idx} className="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-300">
                  ...
                  <span>
                    {typeof stepItem === 'string'
                      ? stepItem
                      : stepItem?.step
                      ? `${stepItem.step}: ${stepItem.calculation || ''} ${stepItem.result ? `→ ${stepItem.result}` : ''}`
                      : JSON.stringify(stepItem)}
                  </span>
                </div>
              ))}
            </div>
            {(calculator.example.conclusion || (calculator.example as any).result) && (
              <div className="pt-2 border-t border-indigo-200/60 dark:border-indigo-900/60 text-xs font-bold text-indigo-900 dark:text-indigo-300">
                {calculator.example.conclusion ? `Conclusion: ${calculator.example.conclusion}` : `Final Result: ${(calculator.example as any).result}`}
              </div>
            )}
          </div>
        </section>
        ...
        <div className="space-y-1">
          <h4 className="font-bold">
            {calculator.disclaimerType === 'health'
              ...
              : calculator.disclaimerType === 'financial'
